...
CASES: List[CaseDef] = [
    ...
    CaseDef(
        ...
        category="internal",
        status="reserved",
        description="Session5 内部主线 E2E 场景 1（预留）。",
        upstream="reserved",
        session="session5",
    ),
    CaseDef(
        ...
        category="internal",
        status="reserved",
        description="Session5 内部主线 E2E 场景 2（预留）。",
        upstream="reserved",
        session="session5",
    ),
    CaseDef(
        ...
        category="internal",
        status="reserved",
        description="Session5 内部主线 E2E 场景 3（预留）。",
        upstream="reserved",
        session="session5",
    ),
    CaseDef(
...
def evaluate_case(case_def: CaseDef, upstream_results: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if case_def.case_id == "M3-S4-HAPPY-001":
        ...
        # 预留 case 不在 Session4 执行，进入此分支即视为策略漏洞。
        passed = False
        notes.append("reserved case should not be evaluated in Session4")
...
def select_upstreams(selected_case_ids: Iterable[str]) -> Set[str]:
    ...
    for case_id in selected_case_ids:
        ...
        if case_def.upstream in {"m3-runtime", "m1-runtime"}:
    ...
    for case_id in selected_case_ids:
        ...
        if case_def.status == "reserved":
            case_payload = {
                ...
                "status": "fail",
                "details": "reserved case is not executable in Session4",
                "natural_language_conclusion": "该用例属于 Session5/Session6 预留项，Session4 强制执行触发 Fail-Closed。",
