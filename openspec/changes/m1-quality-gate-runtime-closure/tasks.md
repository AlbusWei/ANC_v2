...
- [ ] 0.4 每个线程至少完成 1 次 Entire sync + 1 次独立提交，并校验 `Entire-Checkpoint` trailer。
...
- [ ] 2.1 基于 Thread-0 计划输出 M1 运行收口差距清单。
- [ ] 2.2 明确主链路、异常链路、Fail-Closed、回退路径的缺口与优先级。
- [ ] 2.3 产出 Thread-2 可执行输入清单。
...
- [ ] 3.1 将 Thread-1 差距项映射到设计/契约文档。
- [ ] 3.2 更新 OpenSpec 设计与任务拆解，使 Thread-3 有明确实现范围。
- [ ] 3.3 记录线程交接物与风险清单。
...
- [ ] 4.1 按 Thread-2 锁定范围实施最小实现。
- [ ] 4.2 增补必要测试与 Fail-Closed 分支验证。
- [ ] 4.3 输出 Thread-4 回归输入（测试入口、已知风险、回退路径）。
...
- [ ] 5.1 执行回归并记录关键现象、风险判断与是否满足准入。
- [ ] 5.2 对未通过项做最小修正并保持独立提交。
- [ ] 5.3 形成 Thread-5 收口输入包。
...
- [ ] 6.1 汇总 Thread-0~4 证据并完成 OpenSpec 收口说明。
- [ ] 6.2 同步施工平面对应条目并确保状态一致。
- [ ] 6.3 完成最终门禁核验，未满足则 Fail-Closed。
