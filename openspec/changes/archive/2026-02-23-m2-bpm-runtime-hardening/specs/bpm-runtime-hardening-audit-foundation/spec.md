...
- **AND** `docs/design/modules/evidence/bpm-runtime/` exists with pre-created evidence placeholders
...
- **THEN** one new JSONL record is appended to `docs/design/modules/evidence/bpm-runtime/checkpoint_commit_map.jsonl`
...
- **THEN** `docs/design/modules/evidence/bpm-runtime/m6_live_regression_plan.md` specifies round_id naming, git-range collection, checkpoint/commit reconciliation, and pass criteria
