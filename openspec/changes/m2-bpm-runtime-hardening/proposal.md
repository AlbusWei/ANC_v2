...
- 初始化测试入口与回归脚手架（`tests/m2-bpm-runtime/TEST.md`、`tests/m2-bpm-runtime/run_post_dev_regression.py`）。
- 初始化证据目录及文件（含 `checkpoint_commit_map.jsonl`、`git_range.txt`、`m6_live_regression_plan.md`）。
- 在施工平面将该变更标记为 In Progress，显式保持 Q-001 未关闭。
...
- `construction-plane`: Update execution status tracking for this change to In Progress while preserving unresolved gate Q-001.

## Impact

- Affected docs: `docs/architecture/construction_plane.md`, `docs/design/modules/evidence/bpm-runtime/*`.
- Affected test harness: `tests/m2-bpm-runtime/*`.
