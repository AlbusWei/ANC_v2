#!/usr/bin/env python3
"""full-development 运行级协作骨架执行器（phase 分发优先版）。
...
class FullDevelopmentError(RuntimeError):
    """Fail-closed runtime error for full-development."""
...
def repo_root() -> Path:
    ...
    if proc.returncode != 0:
        raise FullDevelopmentError("not inside a git repository")
...
def load_json(path: Path) -> Dict[str, Any]:
    try:
        ...
        raise FullDevelopmentError(f"missing file: {path}") from exc
    except json.JSONDecodeError as exc:
        raise FullDevelopmentError(f"invalid json: {path}: {exc}") from exc
    if not isinstance(payload, dict):
        raise FullDevelopmentError(f"json root must be object: {path}")
    return payload
...
PHASE_OUTPUT_FIELDS: Dict[str, List[str]] = {
    ...
    "p8": ["improvement_plan_ref", "retro_report_ref"],
}


PHASE_INPUT_KEYS: Dict[str, List[str]] = {
    "p1": ["objective_context_ref", "input_payload"],
    "p2": ["objective_ref", "scope_baseline_ref"],
    "p3": ["spec_ref", "superpower_ref"],
    "p4": ["spec_ref", "test_plan_ref"],
    "p5": ["candidate_artifacts_ref", "preparation_bundle_ref", "superpower_ref"],
    "p6": ["final_gate_verdict_ref", "lifecycle_target", "superpower_ref"],
    "p7": ["candidate_artifacts_ref", "final_gate_verdict_ref", "lifecycle_transition_ref", "registry_sync_ref"],
    "p8": ["release_package_ref", "final_gate_verdict_ref", "lifecycle_transition_ref"],
}
...
def dispatch_phase(
    *,
    ...
    if not instance_id:
        raise FullDevelopmentError(f"{phase_id}_dispatch_missing_instance_id")
    ...
    if not session_id:
        raise FullDevelopmentError(f"{phase_id}_dispatch_missing_session_id")
    ...
    if dispatch_openclaw:
        if not dispatch_executed:
            raise FullDevelopmentError(f"{phase_id}_dispatch_not_executed")
        if dispatch_rc != 0:
            raise FullDevelopmentError(f"{phase_id}_dispatch_nonzero_rc:{dispatch_rc}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run full-development collaboration skeleton")
    ...
    parser.add_argument(
        ...
        default="tmp/m2-bpm-runtime/full-development/phase-dispatch/instances",
...
def main() -> int:
    ...
    try:
        required = ["objective_context_ref", "input_payload", "target_asset_type", "lifecycle_target", "superpower_ref"]
        missing = [key for key in required if key not in request]
        if missing:
            raise FullDevelopmentError(f"missing required input fields: {','.join(missing)}")

        manifest = load_json(resolve_path(root, "processes/meta/full-development/process.json"))
        phases = manifest.get("phases")
        if not isinstance(phases, list) or not phases:
            raise FullDevelopmentError("full-development manifest phases empty")

        for phase in phases:
            if not isinstance(phase, dict):
                raise FullDevelopmentError("invalid phase definition")
            phase_id = str(phase.get("phase_id") or "")
            actor = str(phase.get("actor") or "unknown")
            if not phase_id:
                raise FullDevelopmentError("phase_id missing in manifest")
        ...
        if missing_outputs:
            raise FullDevelopmentError(f"missing produced outputs: {','.join(missing_outputs)}")
        ...
        output = {
            ...
            "improvement_plan_ref": produced_refs.get("improvement_plan_ref", ""),
            "retro_report_ref": produced_refs.get("retro_report_ref", ""),
            "reasons": [
                "full_development_collaboration_skeleton_executed",
