...
该流程用于处理 quality gate 产出的 hold 案例，目标是把“卡住”变成可诊断、可处置、可升级的治理路径。它在系统中承担异常恢复中枢，防止流程长期悬挂或无责任归属。
...
- 执行角色：`qa`
- 阶段目的：本阶段围绕以下业务动作推进：收集最小进展信号集。
- 输入语义：本阶段主要消费以下输入：hold_case_ref + runtime_log_ref + execution_state_ref。
- 完成标准：完成判据：必须产出 progress_signals_ref，并满足“三类进展信号已收集或已明确记录失败原因”。
- 交接说明：交接要求：将 progress_signals_ref 交接给 p2。
...
- 执行角色：`qa`
- 阶段目的：本阶段围绕以下业务动作推进：对 hold 场景进行处置分级。
- 输入语义：本阶段主要消费以下输入：progress_signals_ref + triage_policy_ref。
- 完成标准：完成判据：必须产出 triage_action，并满足“分诊动作明确且符合策略”。
- 交接说明：交接要求：将 triage_action 交接给 p3。
...
- 执行角色：`qa`
- 阶段目的：本阶段围绕以下业务动作推进：执行选定的分诊动作。
- 输入语义：本阶段主要消费以下输入：triage_action + triage_report_ref。
- 完成标准：完成判据：必须产出 action_execution_ref，并满足“动作执行与分诊决策一致”。
- 交接说明：交接要求：将 action_execution_ref 交接给 p4。
...
- 执行角色：`bpm`
- 阶段目的：本阶段围绕以下业务动作推进：维护运行时健康与恢复状态。
- 输入语义：本阶段主要消费以下输入：action_execution_ref + runtime_health_policy_ref。
- 完成标准：完成判据：必须产出 health_maintenance_ref，并满足“健康结果可追溯且可恢复性明确”。
- 交接说明：交接要求：将 health_maintenance_ref 交接给 p5。
...
- 执行角色：`bpm`
- 阶段目的：本阶段围绕以下业务动作推进：收敛 hold 案例或继续升级。
- 输入语义：本阶段主要消费以下输入：triage_action + health_maintenance_ref。
- 完成标准：完成判据：必须产出 hold_resolution_ref，并满足“hold 案例按治理链路收敛或升级”。
