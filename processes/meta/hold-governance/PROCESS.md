...
- 执行角色：`qa`
- 阶段目的：收集最小进展信号集。
- 输入语义：hold_case_ref + runtime_log_ref + execution_state_ref。
...
- 执行角色：`bpm`
- 阶段目的：维护运行时健康与恢复状态。
- 输入语义：action_execution_ref + runtime_health_policy_ref。
...
- 输入语义：triage_action + health_maintenance_ref。
- 完成标准：必须产出 hold_resolution_ref，并满足“hold 案例按治理链路收敛或升级”。
