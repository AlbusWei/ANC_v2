...
class HotfixError(RuntimeError):
    """Fail-closed runtime error for hotfix."""
...
def repo_root() -> Path:
    ...
    if proc.returncode != 0:
        raise HotfixError("not inside a git repository")
...
def load_json(path: Path) -> Dict[str, Any]:
    try:
        ...
        raise HotfixError(f"missing file: {path}") from exc
    except json.JSONDecodeError as exc:
        raise HotfixError(f"invalid json: {path}: {exc}") from exc
    if not isinstance(payload, dict):
        raise HotfixError(f"json root must be object: {path}")
    return payload
...
PHASE_OUTPUT_FIELDS: Dict[str, List[str]] = {
    "p1": ["hotfix_objective_ref", "impact_scope_ref", "rollback_direction_ref"],
    "p2": ["hotfix_scope_baseline_ref", "spec_ref"],
    ...
    "p7": ["release_package_ref", "rollback_bundle_ref"],
}
...
def dispatch_phase(
    *,
    ...
    if not instance_id:
        raise HotfixError(f"{phase_id}_dispatch_missing_instance_id")
    ...
    if not session_id:
        raise HotfixError(f"{phase_id}_dispatch_missing_session_id")
    ...
    if dispatch_openclaw:
        if not dispatch_executed:
            raise HotfixError(f"{phase_id}_dispatch_not_executed")
        if dispatch_rc != 0:
            raise HotfixError(f"{phase_id}_dispatch_nonzero_rc:{dispatch_rc}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run hotfix collaboration skeleton")
    parser.add_argument("--input", required=True, help="Input JSON path")
    parser.add_argument("--output", required=True, help="Output JSON path")
    parser.add_argument(
        "--evidence-dir",
        default="",
        help="Evidence directory path (repo-relative). Default runtime_data/execution/evidence/bpm-runtime/w3d_hotfix_refactor_cases/<run_id>/hotfix/execution",
    )
    ...
    parser.add_argument(
        ...
        default="tmp/m2-bpm-runtime/hotfix/phase-dispatch/instances",
...
def main() -> int:
    ...
    run_id = args.run_id.strip() or datetime.now(timezone.utc).strftime("hotfix-%Y%m%dT%H%M%SZ")
    evidence_dir = resolve_path(
        root,
        args.evidence_dir.strip() or f"runtime_data/execution/evidence/bpm-runtime/w3d_hotfix_refactor_cases/{run_id}/hotfix/execution",
    )
    ...
    try:
        ...
        manifest = load_json(resolve_path(root, "processes/meta/hotfix/process.json"))
        phases = manifest.get("phases")
        if not isinstance(phases, list) or not phases:
            raise HotfixError("hotfix manifest phases empty")

        for phase in phases:
            if not isinstance(phase, dict):
                raise HotfixError("invalid phase definition")
            phase_id = str(phase.get("phase_id") or "")
            actor = str(phase.get("actor") or "unknown")
            if not phase_id:
                raise HotfixError("phase_id missing in manifest")
        ...
        if missing_outputs:
            raise HotfixError(f"missing produced outputs: {','.join(missing_outputs)}")
        ...
        output = {
            ...
            "hotfix_objective_ref": produced_refs.get("hotfix_objective_ref", ""),
            "impact_scope_ref": produced_refs.get("impact_scope_ref", ""),
            "rollback_direction_ref": produced_refs.get("rollback_direction_ref", ""),
            "hotfix_scope_baseline_ref": produced_refs.get("hotfix_scope_baseline_ref", ""),
            ...
            "release_package_ref": produced_refs.get("release_package_ref", ""),
            "rollback_bundle_ref": produced_refs.get("rollback_bundle_ref", ""),
            "reasons": [
                "hotfix_collaboration_skeleton_executed",
