...
- 阶段目的：将事件归一为标准触发包。
- 输入语义：event payload。
- 完成标准：必须产出 canonical_trigger_ref，并满足“事件字段满足最小标准契约”。
...
- 完成标准：必须产出 dedupe_decision，并满足“去重决策具备确定性且可追溯”。
- 交接说明：将 dedupe_decision 交接给 p3。
...
- 执行角色：`bpm`
- 阶段目的：为命中事件创建运行时实例。
- 输入语义：dedupe_decision。
- 完成标准：必须产出 instance_id，并满足“事件命中后仅产生一个实例”。
...
- 阶段目的：持久化触发回执与可追溯链路。
- 输入语义：instance_id。
- 完成标准：必须产出 trigger_receipt_ref，并满足“触发回执包含去重与分发决策”。
...
- 执行角色：`bpm`
- 阶段目的：证据不足时给出补数或动态补跑决策。
- 输入语义：trigger_receipt_ref。
- 完成标准：必须产出 catchup_decision，并满足“证据缺失时会给出明确补数或升级提示”。
...
- 完成标准：必须产出 escalation_ref，并满足“升级链完整且可审计”。
