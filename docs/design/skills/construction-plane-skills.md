...
- owner：`architect`
- 输入契约：`round_id`, `scope_baseline_ref`, `changed_assets`, `linkage_targets`, `openspec_ref`
...
- runner：`skills/system/openspec-sync/scripts/openspec_sync.sh`
...
2. `system.integration.openspec-sync` 已落盘并进入 registry `review`。
3. `review` 前置（已满足）：
