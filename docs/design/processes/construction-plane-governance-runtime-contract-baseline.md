...
1. `p1 -> AP-032`
   - 输入：`round_id`, `round_goal`, `change_scope_ref`, `changed_assets`, `linkage_targets`, `owner`, `openspec_ref`
   - 输出：`scope_baseline_ref`
2. `p2 -> AP-033`
   - 输入：`round_id`, `scope_baseline_ref`, `linkage_targets`, `changed_assets`, `openspec_ref`
...
4. `p4 -> AP-035`
   - 输入：`round_id`, `round_goal`, `openspec_ref`, `anc_design_refs`, `decision_snapshot_ref`, `sync_actor`, `trigger_mode`, `risk_level`, `checkpoint_count`, `commit_count`, `round_evidence_log_ref`, `output_ref`
   - 输出：`openspec_sync_ref`, `sync_status`, `validate_report_ref`, `status_report_ref`
5. `p5 -> AP-036`
   - 输入：`round_id`, `m6_update_bundle_ref`, `openspec_sync_ref`, `round_evidence_log_ref`, `open_questions_ref`
...
5. 单个 `round_id` 只能绑定单个 `openspec_ref`。
