...
> 版本: v0.4.1 | 层级: P4 | 类型: 复合流程 | process_id: full-development | process_type: dev.internal-productization
...
4. `lifecycle_target`
5. `superpower_ref`（必填，贯通 P3/P5/P6 的执行上下文）
...
| `p3` | `qa` | 围绕规格构建测试准备包，明确后续验证基线。 | spec_ref,superpower_ref | 必须产出 test_plan_ref 与 preparation_bundle_ref。 | 将 test_plan_ref 与 preparation_bundle_ref 交接给 p4。 |
...
| `p5` | `qa` | 执行质量门禁评测并形成统一门禁结论。 | candidate_artifacts_ref,preparation_bundle_ref,superpower_ref | 必须产出 final_gate_verdict_ref，且门禁结论可解释。 | 若门禁通过，则将 final_gate_verdict_ref 交接给 p6。 |
| `p6` | `hr` | 完成生命周期治理与 registry 同步前置校验。 | final_gate_verdict_ref,lifecycle_target,superpower_ref | 必须产出 lifecycle_transition_ref 与 registry_sync_ref。 | 将 lifecycle_transition_ref 与 registry_sync_ref 交接给 p7。 |
