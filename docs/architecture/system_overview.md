...
4. 产品定位（Product Positioning）：以流程为业务价值实现基础的“通用 AI 原生公司框架（Process-First Company OS）”。
5. 定位约束：测试能力属于治理与门禁子系统，不作为产品主叙事。
...
6. 产品叙事纠偏：明确“流程驱动业务价值”为中心，测试定位回归治理支撑。
...
| 模块 | 职责 | 当前优先级 |
|---|---|---|
| M1 验证与门禁体系 | 目标达成判定、Scenario 执行、verdict 归一化、发布门禁裁决 | P0 |
...
3. 治理完整性优先于“快速产能”：先建立可判定、可回滚、可审计机制再扩展开发规模（测试是治理组成）。
