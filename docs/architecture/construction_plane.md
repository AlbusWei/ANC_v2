...
- [ ] 推进 `m3-self-development-e2e-online`：完成 Session6~Session7 运行级测试与双主线 E2E 收口（Session5 内部主线已通过）

## 下一步（Next）

1. 为 P6 原子流程补充 process.json 样板定义。
2. 完成 `review-processes-business` 未闭环项：`governance_bundle` 落地、legacy 语义收敛、canonical 路径策略定稿。
3. 基于 `tests/m3-self-development/run_tc_online.py` 执行 Session6 外部主线 E2E 并沉淀内外复用对照证据。
